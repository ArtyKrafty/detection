...
Другой вариант - запуск через Docker (на М1 MAC OS данный способ может не сработать) - с данным способом много `issues` - не желательно использовать. 
Сработает, если у вас есть `Cuda`:
